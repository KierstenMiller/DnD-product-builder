...
import { type aggulativeStackIndexI, type aggulativeStacksBuildT, type globalRulesI, type modifiersT, type validationLibraryT } from '-/build-your-own/shared/typing/build-your-own.types'
import { findPiece } from '-/build-your-own/workspace-specific-files/aggulative-stacks/utils/builder.util'
import { validateWorkspace } from '-/build-your-own/workspace-specific-files/aggulative-stacks/utils/validation.util'
...
export const AggulativeStacksWorkspace = observer(({ build, globalValidation, validationLibrary }: propsI) => {
  ...
  const validiate = (dropPosition: aggulativeStackIndexI, creatingNewStackOnDrop: boolean) => (
    (validationLibrary?.length === 0 || (!draggingPieceId && !draggingPiece))
      ? true
      : validateWorkspace({
        dropPosition,
  ...
  return (<div className="flex a-i-end">
    {build?.stacks?.map((stack, index) => <Stack
      ...
      isDraggingPieceId={draggingPieceId}
      validDrop={validiate}
