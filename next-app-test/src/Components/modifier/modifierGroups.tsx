...
export const ModifierGroups = observer(({ model, modifiers }: BuildYourOwnPageI) => {
    ...
    const onChange = ({ newSelection }: onChangeI, mod: modifierI) => model.updateConfigSelection({ id: mod.id, key: mod.groupKey, selection: newSelection, value: getOptionValue(mod.id, newSelection) })
    return <>
